...
<template>
	<View class="relative">	
		<div 
			ref="scrollRef"
			class="overflow-y-scroll no-scrollbar pb-2 flex flex-col gap-y-4"
		>
			<div 
				v-for="submission in accumulatedSubmissions"
				:key="submission.edible.id"
				class="bg-dark-tertiary border border-smoke/40 text-chalk rounded-md px-3 py-12"
			>
				<p class="font-medium">{{ submission.edible.information.base.name }}</p>
				<p class="opacity-60">
					{{ submission.edible.information.base.brand }}
				</p>
			</div>
